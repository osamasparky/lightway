...
class InstructorFinderController extends Controller
{
    ...
    private function handleAgeFilter($query, Request $request)
    {
        ...
        if ((!empty($minAge) && is_numeric($minAge)) || (!empty($maxAge) && is_numeric($maxAge))) {
            $userAgeQuery = UserMeta::where('name', 'age');

            if (!empty($minAge) && is_numeric($minAge) && (int)$minAge >= 0) {
                $userAgeQuery->where('value', '>=', (int)$minAge);
            }

            if (!empty($maxAge) && is_numeric($maxAge) && (int)$maxAge >= 0) {
