...
class FileController extends Controller
{
    ...
    private function handleUnZipFile($data, $user_id)
    {
        ...
        if (!$storage->exists($extractPath)) {
            ...
            if ($res === true) {
                $targetDir = public_path($storageExtractPath);
                $blockedExtensions = ['php', 'phtml', 'phar', 'php3', 'php4', 'php5', 'php7', 'sh', 'cgi', 'pl', 'exe', 'htaccess', 'user.ini', 'env'];
                $isValidArchive = true;

                for ($i = 0; $i < $zip->numFiles; $i++) {
                    $entryName = $zip->getNameIndex($i);
                    // Check directory traversal & absolute path
