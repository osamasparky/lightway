...
class PaymentsController extends Controller
{
    ...
    public function paymentByCredit(Request $request, PaymentService $paymentService)
    {
    ...
    // public function paymentRequest(Request $request, PaymentService $paymentService)
    // {
    //     $user = apiAuth();
    //     validateParam($request->all(), [
    ...
    public function paymentRequest(Request $request, PaymentService $paymentService)
{
    ...
    validateParam($request->all(), [
        'gateway' => [
            ...
            function ($attribute, $value, $fail) {
                if ($value !== 'credit'
                    and !PaymentChannel::where('id', $value)
                        ->where('status', 'active')
                        ->exists()
                ) {
                    $fail(trans('api.payment.disabled_gateway'));
                }
            },
        ],
    ...
    if ($order->type === Order::$meeting) {
        ...
        if ($orderItem and $orderItem->reserve_meeting_id) {
            $reserveMeeting = ReserveMeeting::where('id', $orderItem->reserve_meeting_id)->first();
    ...
    try {
        ...
        return apiResponse2(0, 'gift_error', $e->getMessage());
    }
    ...
    try {
        ...
        return apiResponse2(1, 'redirect', trans('api.payment.redirect'), [
