...
class CartController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        validateParam($request->all(),
            [
                'webinar_id' => ['required_without_all:product_id',
                    Rule::exists('webinars', 'id')->where('private', false)
                        ->where('status', 'active')
                ],

                "product_id" => ['required_without_all:webinar_id' , 'exists:products,id'],
                'ticket_id' => 'nullable',
                'quantity' => [Rule::requiredIf(fn() => !empty($request->product_id)), 'min:1'],
            ]
        );
        ...
        if($webinar_id){
            ...
            $this->storeWebinarToCart($webinar_id , $user , $ticket_id);
        }

        if($product_id){
            
            $this->storeProductToCart($product_id , $quantity , $user , $specifications);
    ...
    private function storeWebinarToCart($webinar_id , $user , $ticket_id){
    ...
    private function storeProductToCart($product_id , $quantity , $user , $specifications){
    ...
    public function checkout(Request $request)
    {
        ...
        if ($carts->isEmpty()) {
            return apiResponse2(
                0,
                'empty_cart',
                trans('api.payment.empty_cart')
            );
        }
        //Physical product check
        $hasPhysicalProduct = $carts->where(
            'productOrder.product.type',
            Product::$physical
        );

        $checkAddressValidation = $hasPhysicalProduct->count() > 0;

        if (
            empty(getStoreSettings('show_address_selection_in_cart'))
            or !empty(getStoreSettings('take_address_selection_optional'))
        ) {
        ...
        if (!empty($order)) {
            $order->load([
                ...
                'orderItems.registrationPackage',
                'orderItems.reserveMeeting.meeting',
            ]);
        }

        if (empty($order) or $order->total_amount <= 0) {
            return $this->handlePaymentOrderWithZeroTotalAmount($order);
        }
        //Razorpay check
        $razorpay = false;
        ...
        foreach ($paymentChannels as $paymentChannel) {
            if (
                ...
                and (
                    !$isMultiCurrency
                    or in_array(currency(), $paymentChannel->currencies)
                )
            ) {
                $razorpay = true;
                break;
        ...
        $totalCashbackAmount = $this->getTotalCashbackAmount(
            $carts,
            $user,
            $calculate['sub_total']
        );
    ...
    private function handleOrderPrices($cart, $user, $taxIsDifferent = false)
    {
        ...
        if (!empty($cart->webinar_id) or !empty($cart->bundle_id)) {
            ...
            $subTotal += $price;
        } elseif (!empty($cart->subscribe_id)) {
            ...
            if (!empty($subscribe)) {
                ...
                if ($subscribe->getPrice() < $subscribe->price) {
                    $discount = $price - $subscribe->getPrice();
                }
