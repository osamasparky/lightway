...
class Webinar extends Model implements TranslatableContract
{
    ...
    public function getSaleItem($user = null)
    {
        ...
        if (!empty($user)) {
            $sale = Sale::query()->where(function ($query) use ($user) {
    ...
    public function getFilesLearningProgressStat($userId = null)
    {
        ...
        if ($count > 0 && !empty($userId)) {
            $passed = CourseLearning::where('user_id', $userId)
    ...
    public function getTextLessonsLearningProgressStat($userId = null)
    {
        ...
        if ($count > 0 && !empty($userId)) {
            $passed = CourseLearning::where('user_id', $userId)
                ->whereIn('text_lesson_id', $textLessons)
                ->count();
        }
    ...
    public function getAssignmentsLearningProgressStat($userId = null)
    {
        ...
        if ($count > 0 && !empty($userId)) {
            $passed = WebinarAssignmentHistory::whereIn('assignment_id', $assignments)
    ...
    public function getQuizzesLearningProgressStat($userId = null)
    {
        ...
        if ($count > 0 && !empty($userId)) {
            $passed = QuizzesResult::whereIn('quiz_id', $quizzes)
    ...
    {
        return $this->hasMany('App\Models\SpecialOffer', 'webinar_id', 'id');
    }
    ...
    public function activeSpecialOffer()
    {
        ...
        if ($this->relationLoaded('specialOffers')) {
            $offer = $this->specialOffers->first(function ($item) use ($time) {
                return $item->status === SpecialOffer::$active
                    && $item->from_date < $time
                    && $item->to_date > $time;
            });
            return $offer ?? false;
