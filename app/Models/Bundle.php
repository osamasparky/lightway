...
class Bundle extends Model implements TranslatableContract
{
    ...
    public function checkUserHasBought($user = null, $checkExpired = true): bool
    {
        ...
        if (!empty($user)) {
            $sale = Sale::where(function ($query) use ($user) {
            ...
            if (!empty($sale)) {
                ...
                if ($sale->payment_method == Sale::$subscribe) {
                    $subscribe = $sale->getUsedSubscribe($user->id, $sale->bundle_id, 'bundle_id');
    ...
    {
        return $this->hasMany('App\Models\SpecialOffer', 'bundle_id', 'id');
    }

    public function specialOffer()
    {
        return $this->hasOne('App\Models\SpecialOffer', 'bundle_id', 'id')
    ...
    {
        if ($this->relationLoaded('specialOffer')) {
