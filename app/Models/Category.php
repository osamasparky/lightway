...
class Category extends Model implements TranslatableContract
{
    ...
    public function getUrl()
    {
        ...
        if (!empty($this->parent_id) && $this->relationLoaded('category') && !empty($this->category)) {
            $url .= $this->category->slug . '/';
        } elseif (!empty($this->parent_id) && !empty($this->category)) {
            $url .= $this->category->slug . '/';
        }
            ...
            return self::whereNull('parent_id')
                ->with([
                    ...
                    'subCategories' => function ($query) {
                        $query->orderBy('order', 'asc')->with(['translations', 'category']);
                    },
                ])
                ->orderBy('order', 'asc')
                ->get();
