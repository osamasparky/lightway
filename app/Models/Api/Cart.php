...
class Cart extends Model
{
    ...
    public function getPriceAttribute()
    {
        ...
        if ($this->reserve_meeting_id and $this->reserveMeeting) {
            return $this->reserveMeeting->paid_amount;
        }

        if ($this->product_order_id and $this->productOrder and $this->productOrder->product) {
            return $this->productOrder->product->price * $this->productOrder->quantity;
        }

        if ($this->subscribe_id and $this->subscribe) {
            return $this->subscribe->price;
        }

        if ($this->installment_payment_id and $this->installmentPayment) {
            return $this->installmentPayment->amount;
        }
