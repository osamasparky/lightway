...
class Setting extends Model implements TranslatableContract
{
    ...
    protected static function booted()
    {
        static::saved(function ($setting) {
            if (!empty($setting->name)) {
                cache()->forget('settings.' . $setting->name . '.en');
                cache()->forget('settings.' . $setting->name . '.ar');
                cache()->forget('settings.' . $setting->name . '.' . app()->getLocale());
            }
        });
    }
    ...
    static function getSetting(&$static, $name, $key = null)
    {
        ...
        $cacheKey = 'settings.' . $name . '.' . $locale;

        if (!isset($static) || !is_array($static) || !array_key_exists($locale, $static)) {
            if (!is_array($static)) {
                $static = [];
            }
            $static[$locale] = cache()->remember($cacheKey, 24 * 60 * 60, function () use ($name, $locale) {
                $setting = self::where('name', $name)->with('translations')->first();
                $val = [];
                if ($setting) {
                    $raw = getTranslateAttributeValue($setting, 'value', $locale);
                    if (!empty($raw)) {
        ...
        if (!empty($value) and !empty($key) and is_array($value)) {
            return $value[$key] ?? null;
        }

        if (!empty($key) and (empty($value) or !is_array($value) or count($value) < 1)) {
            return '';
        }
