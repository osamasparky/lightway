...
class Channel extends BasePaymentChannel implements IChannel
{
    ...
    private function handleAccessToken()
    {
        ...
        try {
            ...
            Log::error('Payment gateway error [Instamojo]', ['error' => is_object($e) && method_exists($e, 'getMessage') ? $e->getMessage() : $e]); throw new \Exception('Payment processing failed');
        }

        Log::error('Payment gateway error [Instamojo]', ['error' => is_object($data) && method_exists($data, 'getMessage') ? $data->getMessage() : $data]); throw new \Exception('Payment processing failed');
    }
    ...
    public function paymentRequest(Order $order)
    {
        ...
        try {
            ...
            Log::error('Payment gateway error [Instamojo]', ['error' => is_object($response) && method_exists($response, 'getMessage') ? $response->getMessage() : $response]); throw new \Exception('Payment processing failed');
    ...
    public function verify(Request $request)
    {
        ...
        try {
            ...
            Log::error('Payment gateway error [Instamojo]', ['error' => is_object($e, 1) && method_exists($e, 1, 'getMessage') ? $e, 1->getMessage() : $e, 1]); throw new \Exception('Payment processing failed');
