...
trait TelebirrIntegrationTrait
{
    ...
    public function applyFabricToken()
    {
        ...
        Log::error('Payment gateway error [Telebirr]', ['error' => is_object($response) && method_exists($response, 'getMessage') ? $response->getMessage() : $response]); throw new \Exception('Payment processing failed');
...
Log::error('Payment gateway error [Telebirr]', ['error' => is_object($url) && method_exists($url, 'getMessage') ? $url->getMessage() : $url]); throw new \Exception('Payment processing failed');
        return $url;
    }
...
-----END PRIVATE KEY-----'; // dont format. this is correct
        ...
        if (!$privateKey) {
            Log::error('Payment gateway error [Telebirr]', ['error' => is_object("Error loading PrivateKey") && method_exists("Error loading PrivateKey", 'getMessage') ? "Error loading PrivateKey"->getMessage() : "Error loading PrivateKey"]); throw new \Exception('Payment processing failed');
        }
