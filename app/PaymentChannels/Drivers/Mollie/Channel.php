...
class Channel extends BasePaymentChannel implements IChannel
{
    ...
    public function paymentRequest(Order $order)
    {
        ...
        try {
            ...
            Log::error('Payment gateway error [Mollie]', ['error' => is_object($response) && method_exists($response, 'getMessage') ? $response->getMessage() : $response]); throw new \Exception('Payment processing failed');
            if ($response->isRedirect()) {
                return $response->redirect();
            }

        } catch (\Exception $exception) {
            Log::error('Payment gateway error [Mollie]', ['error' => is_object($exception) && method_exists($exception, 'getMessage') ? $exception->getMessage() : $exception]); throw new \Exception('Payment processing failed');
            throw new \Exception($exception->getMessage(), $exception->getCode());
        }
        Log::error('Payment gateway error [Mollie]', ['error' => is_object(1) && method_exists(1, 'getMessage') ? 1->getMessage() : 1]); throw new \Exception('Payment processing failed');
        $toastData = [
