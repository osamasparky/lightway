@php
    ...
    $templateTranslation = !empty($template) ? $template->translate($currentLocale) : null;
    $templateTitle = (!empty($templateTranslation) and !empty($templateTranslation->title)) ? $templateTranslation->title : (!empty($template) ? $template->title : old('title'));
    $templateBody = (!empty($templateTranslation) and !empty($templateTranslation->body)) ? $templateTranslation->body : (!empty($template) ? $template->body : '');

    $templateElements = [];
    if (!empty($templateTranslation) and !empty($templateTranslation->elements)) {
        $templateElements = is_string($templateTranslation->elements) ? json_decode($templateTranslation->elements, true) : $templateTranslation->elements;
    } elseif (!empty($template) and !empty($template->elements)) {
        $templateElements = is_string($template->elements) ? json_decode($template->elements, true) : $template->elements;
    }
@endphp
...
<div class="card">
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ $templateTitle }}">
                ...
                <div data-element="{{ $element }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
                    <div class="d-flex align-items-center justify-content-between " role="tab" id="{{ $element }}_accordion">
                        ...
                        <div class="d-flex align-items-center">
                            @php
                                $isActiveElement = (!empty($templateElements) and !empty($templateElements[$element]) and !empty($templateElements[$element]['enable']) and $templateElements[$element]['enable'] == 'on');
                            @endphp
                    ...
                    <div id="{{ $element }}_accordion_collapse" aria-labelledby="{{ $element }}_accordion" class="collapse" role="tabpanel">
                        <div class="panel-collapse text-gray">
                            ...
                            @foreach($elementValues as $elementValue)
                                @php
                                    $storedData = (!empty($templateElements) and !empty($templateElements[$element]) and isset($templateElements[$element][$elementValue['name']])) ? $templateElements[$element][$elementValue['name']] : null;
                                @endphp
        ...
        <input type="hidden" name="template_contents" class="js-template-contents" value="{{ $templateBody }}">
