...
@section('content')
    <section class="course-cover-container {{ empty($activeSpecialOffer) ? 'not-active-special-offer' : '' }}">
        <img loading="lazy" src="{{ $bundle->getImageCover() }}" class="img-cover course-cover-img" alt="{{ $bundle->title }}"/>
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    <div class="course-img {{ $bundle->video_demo ? 'has-video' :'' }}">
                        ...
                        <img loading="lazy" src="{{ $bundle->getImage() }}" class="img-cover" alt="">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/cart/store" method="post">
                            ...
                            {{-- <div class="mt-20 d-flex flex-column">
                                ...
                                {{-- @if($canSale and $bundle->canUseSubscribe())
                           ...
                           {{-- NEW CONDITIONS FOR THE ADD TO CART, BUY NOW, BUY BUTTONS --}}
                            ...
                            <div class="mt-20 d-flex flex-column">
                                @if(!empty($bundle->price) and $bundle->price > 0)
                                    <button type="{{ $canPurchase ? 'submit' : 'button' }}" @if(!$canPurchase) disabled @endif class="btn btn-primary">
                                        @if(!$canPurchase)
                                            {{ trans('update.disabled_add_to_cart') }}
                                        @else
                                            {{ trans('public.add_to_cart') }}
                                    ...
                                    @if($canPurchase and !empty($bundle->points))
                                        <a href="{{ !(auth()->check()) ? '/login' : '#' }}" class="{{ (auth()->check()) ? 'js-buy-with-point' : '' }} btn btn-outline-warning mt-20 {{ (!$canPurchase) ? 'disabled' : '' }}" rel="nofollow">
                                            {!! trans('update.buy_with_n_points',['points' => $bundle->points]) !!}
                                        </a>
                                    @endif

                                    @if($canPurchase and !empty(getFeaturesSettings('direct_bundles_payment_button_status')))
                                        <button type="button" class="btn btn-outline-danger mt-20 js-bundle-direct-payment">
                                            {{ trans('update.buy_now') }}
                                        </button>
                                    @endif
                                @else
                                    <a href="{{ $canPurchase ? '/bundles/'. $bundle->slug .'/free' : '#' }}" class="btn btn-primary @if(!$canPurchase) disabled @endif">{{ trans('update.enroll_on_bundle') }}</a>
                                @endif
