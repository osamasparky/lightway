            ...
            <div class="row">
                        ...
                        @if(!$isMultiCurrency or (!empty($paymentChannel->currencies) and in_array($userCurrency, $paymentChannel->currencies)))
                            <div class="col-6 col-lg-4 mb-40 charge-account-radio">
                                ...
                                <label for="{{ $paymentChannel->title }}" class="rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                                    <img loading="lazy" src="{{ $paymentChannel->image }}" width="120" height="60" alt="">
                ...
                <div class="col-6 col-lg-4 mb-40 charge-account-radio">
                    ...
                    <label for="offline" class="rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                        <img loading="lazy" src="/assets/default/img/activity/pay.svg" width="120" height="60" alt="">
            ...
            <div class="modal fade" id="giftModal" tabindex="-1">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content p-20">
                        ...
                        <h4 class="mb-20">{{ trans('financial.who_is_the_purchase_for?') }}</h4>

                        @foreach($order->orderItems as $item)
                          @php
                            $webinar = $item->webinar ?? (!empty($item->webinar_id) ? \App\Models\Webinar::find($item->webinar_id) : null);

                            $forceGift = !empty($webinar) ? $webinar->checkUserHasBought(auth()->user()) : false;
                          @endphp

                            <div class="mb-20 border p-15 rounded">
                                <strong>{{ $item->title ?? 'Item' }}</strong>

                                <div class="mt-10">
                                    @if(!$forceGift)
                                    <label>
                                      {{-- <input type="radio" name="sale_type[{{ $item->id }}]" value="self" checked> --}}
                                      {{-- <input type="radio"
                                        name="sale_type[{{ $item->id }}]"
                                        value="self"
                                        {{ !$forceGift ? 'checked' : '' }}
                                        {{ $forceGift ? 'disabled' : '' }}> --}}
                                        <input type="radio"
                                        name="sale_type[{{ $item->id }}]"
                                        value="self">  
                                       {{ trans('financial.for_myself') }}
                                    </label>
                                     @endif
                                    <label class="ml-15">
                                        {{-- <input type="radio" name="sale_type[{{ $item->id }}]" value="other"> --}}
                                        <input type="radio"
                                          name="sale_type[{{ $item->id }}]"
                                          value="other"
                                          {{ $forceGift ? 'checked' : '' }}>                       
                                        {{ trans('financial.for_someone_else') }}
                                    </label>
                                </div>

                                {{-- <div class="gift-form d-none mt-10" id="gift-form-{{ $item->id }}"> --}}
                                    <div id="gift-form-{{ $item->id }}" class="gift-form mt-10 {{ $forceGift ? '' : 'd-none' }}">                       
                                    <input type="text" name="gift_user[{{ $item->id }}][full_name]" class="form-control mt-5" placeholder="Full Name">
                                    <input type="email" name="gift_user[{{ $item->id }}][email]" class="form-control mt-5" placeholder="Email">
                                    <input type="password" name="gift_user[{{ $item->id }}][password]" class="form-control mt-5" placeholder="Password">
                                </div>
                            </div>
                        @endforeach

                        <button type="button" id="confirmGift" class="btn btn-primary mt-10">
                            {{ trans('financial.continue_to_payment') }}
                        </button>
